...
class Address(UnicodeProperty):
    ...
    def _get_address_lines(self):
        ...
        if self.ir:
            address_line.append(_('CIF/NIF: %s') % self.ir)
...
class Invoice(UnicodeProperty):
    ...
    def __init__(self, client, provider, creator, irpf_rate=Decimal('0.00'), iva_rate=Decimal('21.00'), date=None):
        ...
        self.iva_rate = Decimal(iva_rate)
        self.date = date
    ...
    def irpf(self):
        """ Total sum price including taxes. """
        return self.price * (self.irpf_rate / Decimal(100))

    @property
    def base_amount(self):
        """ Suma base de todos los artículos sin impuestos. """
        return sum(item.total for item in self.items)

    @property
    def total_iva(self):
        """ Cálculo del total de IVA. """
        return self.base_amount * (self.iva_rate / Decimal('100'))

    @property
    def total_irpf(self):
        """ Cálculo del total de IRPF. """
        return -self.base_amount * (self.irpf_rate / Decimal('100'))

    @property
    def total_due(self):
        """ Total sum price including taxes. """
        return self.base_amount + self.total_iva + self.total_irpf
    ...
    def _get_grouped_items_by_tax(self):
        ...
        if[self.irpf_rate]:
            table[self.irpf_rate] = {'total': self.irpf, 'total_tax': self.irpf, 'tax': 0}
        return table
